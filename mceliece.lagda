...
module DistVeritas where
  ...
  dratav x z e₁ e₂ j = sym $ begin
    ...
    length (filterₗ drate $ (e₁ , e₂) ∷ zipₓ x z refl) ≡⟨ lend x z e₁ e₂ j ▹ cong length ⟩
    ...
    lend : ∀ {a} → {A : Set a} → {n : ℕ}
         ...
         → ¬_ $ e₁ ≡ e₂
         → (_≡_
             (filterₗ drate $ (e₁ , e₂) ∷ zipₓ x z refl)
             ((e₁ , e₂) ∷ filterₗ drate (zipₓ x z refl)))
    lend = {!!}
...
module HxVeritas where
  ...
  pavind p T m n d = begin
    ...
    Ix : ∀ {a} → {A : Set a}
       ...
       → (f : Fin n)
       → {x z : A}
       → z ≡_ $ flip lookup f $ lookup (I {n = n} x z) f
    Ix = {!!}
\end{code}
